window.onload=function() {
   var musics=[
     {src:'1.mp3',name:'我要快乐',artistan:'曾一鸣',duration:'03:31'},
     {src:'2.mp3',name:'当爱已成往事',artistan:'曾一鸣',duration:'03:44'},
     {src:'3.mp3',name:'黑色幽默',artistan:'曾一鸣',duration:'02:57'},
     {src:'4.mp3',name:'伤痕',artistan:'曾一鸣',duration:'03:15'},
     {src:'5.mp3',name:'洋葱',artistan:'杨宗纬',duration:'04:38'},
     {src:'6.mp3',name:'空白格',artistan:'杨宗纬',duration:'04:45'}
   ]
   ...
   $prev.on('click',function(){
    ...
    //双击清空播放列表
    $('.liebiaoqingkong').on('dblclick',function(){
        $('.play-list ul').empty();
        audio.src='';
        $('.play-mini .music_name_qumu').text('QQ音乐');
        $('.play-mini .singer_name_qumu').text('享你所爱');
        $('.play-mini .singer_name_time').text('- -:- -');
        //改变对应数字
        $('.play-mini .singer_name_time').text('- -:- -');
        $('.play-mini .open_list span').text(0);
        //移除播放 前进 后退
        $('.btn_down').off('click');
        $('.btn_zanting').off('click');
        $('.btn_top').off('click');
    })
    //播放时更改UI的样式
